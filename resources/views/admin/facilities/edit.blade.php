...
@section('subtitle', 'Edit data fasilitas')
...
<div class="bg-white rounded-xl shadow p-6 max-w-3xl mx-auto">
    ...
    <form action="{{ route('admin.facilities.update', $facility->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <!-- Hidden field to track if coming from show page -->
        @if(url()->previous() === route('admin.facilities.show', $facility->id))
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="3"
                          class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary"
                          placeholder="Deskripsi detail fasilitas..." required>{{ old('description', $facility->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="icon" id="icon"
                    ...
                    <option value="wifi" {{ old('icon', $facility->icon) == 'wifi' ? 'selected' : '' }}>Wi-Fi</option>
                    <option value="socket" {{ old('icon', $facility->icon) == 'socket' ? 'selected' : '' }}>Stop Kontak</option>
                    <option value="baby-chair" {{ old('icon', $facility->icon) == 'baby-chair' ? 'selected' : '' }}>Kursi Bayi</option>
                    <option value="ac" {{ old('icon', $facility->icon) == 'ac' ? 'selected' : '' }}>AC</option>
                    <option value="toilet" {{ old('icon', $facility->icon) == 'toilet' ? 'selected' : '' }}>Toilet</option>
                    <option value="smoking-area" {{ old('icon', $facility->icon) == 'smoking-area' ? 'selected' : '' }}>Area Merokok</option>
                    <option value="parking" {{ old('icon', $facility->icon) == 'parking' ? 'selected' : '' }}>Parkir</option>
                </select>
                
                <!-- Icon Preview -->
                <div id="iconPreview" class="mt-3">
                    <p class="text-sm text-gray-600 mb-2">Icon Terpilih:</p>
                    <div id="selectedIcon" class="inline-flex items-center justify-center w-12 h-12 rounded-lg {{ $facility->icon_color_class }}">
                        <i id="iconPreviewIcon" class="{{ $facility->icon_class }} text-lg"></i>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center space-x-4">
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_available" value="1" 
                               ...
                               {{ old('is_available', $facility->is_available) == '1' ? 'checked' : '' }} required>
                        <span class="ml-2">Tersedia</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_available" value="0"
                               class="text-primary focus:ring-primary" 
                               {{ old('is_available', $facility->is_available) == '0' ? 'checked' : '' }}>
            ...
            <div class="md:col-span-2">
                ...
                <div class="mb-2">
                    <p class="text-sm text-gray-600">Ganti gambar (kosongkan jika tidak ingin mengubah):</p>
                </div>
...
document.getElementById('icon').addEventListener('change', function() {
    ...
    if (selectedValue && iconClasses[selectedValue]) {
        ...
        // Update color - remove old classes, add new ones
